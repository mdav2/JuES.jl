...
@fastmath function do_RCIS(refWfn::Wfn, nroots, algo = "lobpcg", doprint = false)
    ...
    if doprint
        println("Hamiltonian constructed in $dt s")
    end
    ...
    if algo == "lobpcg" || algo == "iter"
        ...
        if doprint
            println("Hamiltonian iteratively solved in $dt s")
        end
    ...
    elseif algo == "svd"
        ...
        if doprint
            println("Hamiltonian diagonalized exactly in $dt s")
        end
    elseif algo == "diag"
        H = Symmetric(H)
        ...
        if doprint
            println(
                "solver ",
                algo,
                " is not supported! Choose from { lobpcg (synonym iter), davidson, svd, diag }",
            )
        end
        return false
    end
...
function so_eri(eri_voov, eri_vvoo, a, j, i, b)
    ...
    (a % 2 == b % 2) * (j % 2 == i % 2) * eri_vvoo[aa, bb, jj, ii]
end

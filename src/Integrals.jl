...
function disk_ao(mints::PyObject,basis::PyObject,name::String="default")
    ...
    while !si.is_done()
        ...
        ao_eri[qf:qf+qn,pf:pf+pn,sf:sf+sn,rf:rf+rn] = permutedims(permutedims(shell,[2,1,3,4]),[1,2,4,3])
        ao_eri[rf:rf+rn,sf:sf+sn,pf:pf+pn,qf:qf+qn] = permutedims(permutedims(shell,[3,2,1,4]),[1,4,3,2])
        ao_eri[sf:sf+sn,rf:rf+rn,pf:pf+pn,qf:qf+qn] = permutedims(permutedims(permutedims(shell,[3,2,1,4]),[1,4,3,2]),[2,1,3,4])
        ao_eri[rf:rf+rn,sf:sf+sn,qf:qf+qn,pf:pf+pn] = permutedims(permutedims(permutedims(shell,[3,2,1,4]),[1,4,3,2]),[1,2,4,3])
        ao_eri[sf:sf+sn,rf:rf+rn,qf:qf+qn,pf:pf+pn] = permutedims(permutedims(shell,[4,2,3,1]),[1,3,2,4])
        si.next()
    end
    return ao_eri
